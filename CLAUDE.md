...
- **Narrow, annotation-only write surface.** The only write paths are rename (`r` — local or function), comment (`;`), and bookmark/tag (`t`), all via `bn` and all in-memory on the live instance. None call `bn save` — persistence to the on-disk `.bndb` is deliberately deferred (see TODO.md). A local rename retexts tokens in place (`apply_local_rename`, no re-decompile) for latency; a function rename returns `Exit::Reload` so the app rebuilds ctx (name maps/callsites must update). Keep writes to naming/annotation — types/structs/graph editing remains an explicit non-goal.
